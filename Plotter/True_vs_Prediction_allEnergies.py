...
counter = 0
for energy in FCSDFs:
  if counter == 0: FCSdataset = FCSDFs[energy]
  else: FCSdataset = pd.concat([FCSdataset,FCSDFs[energy]],ignore_index=True)
  counter += 1
print('INFO: Last 5 rows of FCS DF')
print(FCSdataset.tail(5))
...
for Label in labels:
  ...
  ax = plt.gca()
  plt.hist(Diffs[:,counter],bins=binning,label='Keras')
  plt.hist(FCSdiff[:,counter],bins=binning,label='FCS',color='r',alpha=0.5)
  plt.xlim([-0.75,0.75])
  plt.legend()
  ...
  plt.ylabel('Events')
  plt.savefig('Plots/{}/{}_TruePredictionDiff_{}.pdf'.format(Version,OutBaseName,Label))
  plt.close('all')
  counter += 1
