...
CSV files should be created using MakeCSVfiles_NN.py from https://github.com/jbossios/PrepareCSVFiles
...
(optional) Compare predicted-true extrapolation weight distribution b/w trained network and FCS

$ cd Plotter/

First: Produce CSV files with firstPCAbin values with MakeCSVfiles_FCS.py script from https://github.com/jbossios/PrepareCSVFiles
This is needed to obtain the current extrapolation weights and compare with those predicted by the network
